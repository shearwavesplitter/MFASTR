...
c  ** make sure the essential quantities are defined **
	...
	if (b    .lt.0.) then
            print *, 'ERROR: zreaddataSAC: b    = -ve'
            print *, ' try synch begin on in sac to get to 0'
            stop
        end if
c  take care of negative b later
	if (force_spick) then	
...
c
c  this bit below didn't work for some data so going back to requiring b to
c   be positive
c	if (b    .lt.0.) then
c          print*,'warning: zreaddataSAC: b <0 resetting spick'
c	  spick = spick - b
c          print *,' new b, spick ' ,b,spick
cc          print*, 'new b, spick ' b,spick
c	  if (nint(ppick).ne.-12345) ppick = ppick - b
c	  if (nint(as).ne.-12345) as = as - b
c	  if (nint(fs).ne.-12345) fs = fs - b
c	  ev_time = ev_time -b/86400.
c          b = 0.
c        endif
