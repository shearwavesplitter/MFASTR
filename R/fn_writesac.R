...
{
    sacheadnames = c("delta", "depmin", "depmax", "scale", "odelta", 
        "b", "e", "o", "a", "internal1", "t0", "t1", "t2", "t3", 
        "t4", "t5", "t6", "t7", "t8", "t9", "f", "resp0", "resp1", 
        "resp2", "resp3", "resp4", "resp5", "resp6", "resp7", 
        "resp8", "resp9", "stla", "stlo", "stel", "stdp", "evla", 
        "evlo", "evel", "evdp", "unused1", "user0", "user1", 
        "user2", "user3", "user4", "user5", "user6", "user7", 
        "user8", "user9", "dist", "az", "baz", "gcarc", "internal2", 
        ...
        "internal4", "internal5", "internal6", "npts", "internal7", 
        "internal8", "unused13", "unused14", "unused15", "iftype", 
        "idep", "iztype", "unused16", "iinst", "istreg", "ievreg", 
        "ievtyp", "iqual", "isynth", "unused17", "unused18", 
        "unused19", "unused20", "unused21", "unused22", "unused23", 
        "unused24", "unused25", "unused26", "leven", "lpspol", 
        "lovrok", "lcalda", "unused27", "kstnm", "kevnm[16]", 
        "khole", "ko", "ka", "kt0", "kt1", "kt2", "kt3", "kt4", 
        "kt5", "kt6", "kt7", "kt8", "kt9", "kf", "kuser0", "kuser1", 
...
for (i in 1:10) {
	val <- paste0("t",i-1)
   A1[which(sacheadnames == val)] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == val)]]))
	}
	v <- "depmin"
   A1[which(sacheadnames == v)] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
	v <- "depmax"
   A1[which(sacheadnames == v)] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
	v <- "o"
   A1[which(sacheadnames == v)] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
...
for (i in 4:8) {
	val <- paste0("internal",i)
   A2[which(sacheadnames == val)-70] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == val)]]))
	}
	v <- "lpspol"
   A2[which(sacheadnames == v)-70] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
	v <- "lovrok"
   A2[which(sacheadnames == v)-70] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
	v <- "lcalda"
   A2[which(sacheadnames == v)-70] = as.numeric(as.character(alist$HEAD$values[[which(alist$HEAD$names == v)]]))
...
for (i in 1:10) {
	val <- paste0("kt",i-1)
   B[which(NAMB == val)] = as.character(alist$HEAD$values[[which(alist$HEAD$names == val)]])
	}
#
    ...
    if (is.null(fn)) {
        ...
        sacfn = paste(sep = ".", formatC(yr, width = 4, flag = "0"), 
            formatC(jd, width = 3, flag = "0"), formatC(hr, width = 2, 
                flag = "0"), formatC(mi, width = 2, flag = "0"), 
            formatC(trsec, width = 2, flag = "0"), staname, comp3, 
            "SAC")
    }
    else {
        sacfn = fn
    ...
    zz <- file(sacfn, "wb")
    for (j in 1:70) {
        writeBin(A1[j], zz, size = ifloat, endian = theENDIAN)
    }
    for (j in 1:40) {
        writeBin(as.integer(A2[j]), zz, size = ilong, endian = theENDIAN)
    }
